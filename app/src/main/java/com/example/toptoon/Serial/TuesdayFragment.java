...
public class TuesdayFragment extends BaseMainListFragment {
        ...
        NetworkManager.fetchTopToonItems(new Callback<ApiItems>() {
            ...
            public void onResponse(@NonNull Call<ApiItems> call, @NonNull Response<ApiItems> response) {
                if (response.isSuccessful() && response.body() != null) {
                    List<Integer> mondayIds = response.body().getSerial().getTuesday();
                    List<ApiItems.Webtoon> webtoons = response.body().getWebtoons();
                    List<BaseContentItem> baseContentItems = filterTabContentItemsByWebtoonIds(mondayIds, webtoons);
